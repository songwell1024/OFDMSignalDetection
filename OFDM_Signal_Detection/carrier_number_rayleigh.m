function [carry_num] = carrier_number_rayleigh(yc,N,snr,K,itau,power,fmax,fs,itn)

yc1 = real(MUL_RAYLEIGH(yc,itau,power,itn,length(itau),length(yc),1/fs,fmax,0));
yc1 = awgn(yc1,snr,'measured');
fff =5; % 检测采样率,对信号进行过采样
y1= yc1(ones(fff,1),:); % 码元复制
y1= reshape(y1, 1, fff*length(y1));
sig_2 = y1;
%P = length(sig_2)/N;
P = length(yc)/N;
fft_num = 6000;       %fft点数
